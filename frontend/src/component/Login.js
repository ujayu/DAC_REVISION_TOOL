    ...
    axios
      ...
      .catch((error) => {
        ...
        if(typeof(error.response.data.data) != "undefined"){
          if (error.response.data.data.isAccountLock) {
            setLockoutTime(Math.ceil(error.response.data.data.accountLockTime + 1));
            return;
          }
        }
        setShowFailureMessage(error.response.data.message);
      });
  }
  ...
  useEffect(() => {
    if (lockoutTime > 0) {
      const timer = setInterval(() => {
        setLockoutTime(prevLockoutTime => prevLockoutTime - 1);
      }, 60 * 1000);
          ...
          {!showSuccessMessage && (
            <>
              ...
              <div className="text-center">
                ...
                {lockoutTime > 0 && <p className="text-danger">Account locked. Try again in {lockoutTime} Minute.</p>}
              </div>
            </>
          )}
